...
class CommentController {
    ...
    def ajaxListByStory() {
        def list = Comment.createCriteria().list() {
            story {
                eq("id", params?.storyId as Long)
            }
        }
        
        def toRender = list.collect { c ->
            ["text": c.toString()]
        }
        def result = [result: toRender]
    ...
    def save() {
		...
		if (commentInstance.story)
            redirect(controller: "story", action: "show", id: commentInstance.story?.id)
        else
    ...
    def delete() {
        def commentInstance = Comment.get(params.id)
        def storyId = commentInstance?.story?.id
        if (!commentInstance) {
			flash.message = message(code: 'default.not.found.message', args: [message(code: 'comment.label', default: 'Comment'), params.id])
            redirect(action: "list")
            return
